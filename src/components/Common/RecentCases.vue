<template>
  <div class="cases-area ptb-100">
    <div class="container">
      <div class="section-title" v-if="cases">
        <h2>{{ cases.heading }}</h2>
        <p>{{ cases.shortDesc }}</p>
      </div>

      <CasesStudyItem></CasesStudyItem>
    </div>
...
import CasesStudyItem from "./CasesStudyItem.vue";
...
export default {
  ...
  created: async function () {
    const response = await axios.get(
      `${process.env.VUE_APP_API_URL}recentcase?populate=deep`,
    );
    const {
      data: { attributes },
    } = response.data;
    this.cases = attributes;
  },
};
</script>
