  ...
  <div class="cases-area ptb-100">
    <div class="container">
      <div class="section-title">
        <h2>行业解决方案</h2>
        <p>
          从汽车到制造业，海思特为众多行业客户提供了高效、智能的行业解决方案，帮助他们在数字化浪潮中立于不败之地。
        </p>
      </div>
      <div class="row">
        <div class="col-lg-6" v-for="cas in cases" :key="cas.id">
          <div class="single-cases">
            <div class="cases-image">
              <router-link
                :to="`/case-study-details/${cas.attributes.slug}`"
                style="height: 400px"
              >
                <img
                  :src="cas.attributes.image.data.attributes.url"
                  alt="image"
                />
              </router-link>
            </div>
...
export default {
  ...
  created: async function () {
    const response = await axios.get(
      `${process.env.VUE_APP_API_URL}casestudies?populate=deep`,
    );
